...
@dataclass
class MultipartiteGraphConfig:
    weight: float = 1.0


class MultipartiteGraphTransformer(BaseTransformer):
    """
    ...
    def __init__(
        self,
        configs: Dict[str, MultipartiteGraphConfig],
    ...
    def transform(self, input_data: DataFrame) -> DataFrame:
        ...
        Params:
            input_data: Spark DataFrame
                ...
                or dynamic relationship data with schema [entity_col, attribute_col, value_col, time_col]

        Returns:
            Spark DataFrame Graph edge data with schema [Source, Target, SourceType, TargetType, Weight, (time_col)]
        """
        results = []

        for attribute, attribute_config in self.configs.items():
            graph = graph_edges.generate_bipartite_edges(
                ...
                weight=attribute_config.weight,
            ).cache()
            logger.info(f"Multipartite graph edges for {attribute}: {graph.count()}")
